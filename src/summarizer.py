...
## Decisions
A decision is a choice the participants made in THIS meeting about what they will do. Background facts, org news, and things learned are not decisions and belong in the Summary. State each decision as what the group chose to do. When they rejected one approach in favor of another, state the approach they chose, not the rejection. Begin a line with "Not to" only when the outcome was purely a rejection with nothing adopted in its place. Recording a rejected option as if it was adopted is the worst possible error, and so is stating one person's doubt as a group rejection. State each decision directly, never beginning with "It was decided that", "It is settled that", or "The team". If nothing was decided, write exactly: None captured.
## Action items

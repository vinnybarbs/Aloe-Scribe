...
Telling voices apart within a side uses offline speaker diarization. On Apple Silicon the primary engine is [Senko](https://github.com/narcotic-sh/senko) (CoreML-accelerated, density-based speaker counting, about an hour of audio in under 10 seconds); it was chosen after it matched human-confirmed speaker counts on real meetings where the previous engine over-split. When Senko is unavailable (Windows, or a failed install) the pipeline falls back to sherpa-onnx (pyannote segmentation plus CAM++ embeddings, about 36 MB of models in `~/.cache/aloe-scribe/diarization/`), and if that is also unavailable the transcript still gets channel-level labels: everything on the mic is `M1`, everything remote is `R1`, and the header says so in `speaker_note`.
...
Naming speakers happens live, in the Meeting Notes window that opens with each recording. Type (or click) a person's name while they are talking and the pipeline names their diarized cluster from those timestamped tags — tagging beats any after-the-fact guessing because you can hear the speaker at tag time, and two clusters tagged with the same name merge automatically. The window also holds a timestamped notepad whose entries are saved into a Notes section of the transcript, and a live transcript pane that only autoscrolls while you are at the bottom. When processing finishes, the pane becomes the editable final transcript with one chip button per speaker for click-to-rename, plus free-text editing and a Save button. Untagged speakers keep anonymous labels (a `speaker_channels` header line preserves each name's mic/remote side for the agent), and the Done screen's "Name speakers…" button covers after-the-fact fixes.
